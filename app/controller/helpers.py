"""
Pure data-transformation helpers shared by the API layer and the NiceGUI UI.

These functions contain no UI coupling (no NiceGUI imports, no color/style
logic) and live here so both the REST API (controller/api.py) and the NiceGUI
views (ui/views/) consume the same grouped/flattened representations without
either layer importing from the other.

The NiceGUI-specific style/timeline utilities that this module imports
(aggregate_state, timeline_from_history, merge_timelines) are pure functions
that happen to live in ui/ for historical reasons — they carry no NiceGUI
...
from ..ui.styles import aggregate_state
from ..ui.timeline import merge_timelines, timeline_from_history
