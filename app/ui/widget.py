...
async def render_dashboard_widget(ctx, service: MonitoringService | None):
    ...
    with ui.row().classes("w-full justify-between"):
        with ui.column().classes("items-center gap-0"):
            ui.label(str(total)).classes("text-2xl font-black text-[var(--lx-text)]")
            ui.label("Total").classes(UIStyles.LABEL_MINI)
        with ui.column().classes("items-center gap-0"):
            ui.label(str(up)).classes("text-2xl font-black text-[var(--lx-state-up)]")
            ui.label("Up").classes(UIStyles.LABEL_MINI)
        with ui.column().classes("items-center gap-0"):
            ui.label(str(down)).classes("text-2xl font-black text-[var(--lx-state-down)]")
            ui.label("Down").classes(UIStyles.LABEL_MINI)
        if paused:
            with ui.column().classes("items-center gap-0"):
                ui.label(str(paused)).classes("text-2xl font-black text-[var(--lx-state-paused)]")
                ui.label("Paused").classes(UIStyles.LABEL_MINI)
        if unknown:
            with ui.column().classes("items-center gap-0"):
                ui.label(str(unknown)).classes("text-2xl font-black text-[var(--lx-state-unknown)]")
